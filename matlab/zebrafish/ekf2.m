...
%for i = 1:1:9
tau1 = .28*axi1(9) + (2.8); %Tp
tau2 = 1.2*axi2(9) + (12); %Tm
...
% Use delay equations for time delay term to get 10x10 matrix
    dFdx(:,:,k) = tstep*[[-b,0,0,0,0,0,0,0,0,a];
    ...
    Q(1,1) = abs(a*x(10,k-1) - (b*x(1,k-1)));
     %Q(2,2) = (sqrt((m/(1 + ((x(6,k))^2)/(p0^2)))) - sqrt(abs(c*x(2,k-1))))^2; %multiply by noise coeff
...
var1 = squeeze(Pp(1,1,:));
var2 = squeeze(Pp(2,2,:));
vartotnew9 = [var1,var2]';
save('vartotnew9','vartotnew9')
...
title(['Variance for Tm = ',num2str(tau2,3), ' and Tp = ',num2str(tau1,3)])
legend('p(t)','m(t)')
%end
